...
if [ ! -d "design" ]; then
  ...
  echo "NEXT ACTION: Run \"ship design init\""
  exit 0
fi
...
if [ "$strategy_complete" != "✓ Complete" ]; then
  next_action="Run \"ship design strategy\""
elif [ "$system_complete" != "✓ Complete" ]; then
  next_action="Run \"ship design system\""
elif [ "$screen_features" -le 0 ]; then
  next_action="Run \"ship design screens\""
else
  next_action="Ready for \"ship design export\""
fi
