...
if [ ! -f "apps/web/playwright.config.ts" ]; then
  echo "⏭️  Playwright not configured yet, skipping E2E tests"
  exit 0
fi

if [ ! -d "apps/web/tests" ] || ! ls -1 apps/web/tests >/dev/null 2>&1; then
  echo "⏭️  No Playwright tests found, skipping E2E tests"
  exit 0
fi
